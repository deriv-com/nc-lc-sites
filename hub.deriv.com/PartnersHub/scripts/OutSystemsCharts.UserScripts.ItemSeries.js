        ...
        class p extends o {
                    ...
                    if (c.graphics = g = c.graphics || [], this.chart.styledMode || (m = this.pointAttribs(c, c.selected && "select")), !c.isNull && c.visible) {
                        ...
                        for (let s = 0; s < (c.y || 0); ++s) {
                            if (this.center && this.slots) {
                                ...
                                y = t.x - p / 2, S = t.y - p / 2
                            } else "horizontal" === t.layout ? (y = u % l * n, S = h * Math.floor(u / l)) : (y = n * Math.floor(u / o), S = u % o * h);
                            y += b, S += b, w = Math.round(A - 2 * b), this.options.crisp && (y = Math.round(y) - i, S = Math.round(S) + i), f = {
                                x: y,
                                y: S,
                                width: w,
                                height: w
            ...
            getSlots() {
                let t = this.center,
                    ...
                    l = t[3],
                    n, h, d, p, u, c, f, g, m = 0,
                    y, S = Number.MAX_VALUE,
                    w, M, v, C = (e - l) / e;
                for (; S > a + (M && r ? M.length : 0);) {
                    w = S, s.length = 0, S = 0, M = v, v = [], y = e / ++m / 2, o ? (l = (y - o) / y * e) >= 0 ? y = o : (l = 0, C = 1) : y = Math.floor(y * C);
                ...
                if (!M) return;
                let A = w - this.total - (r ? M.length : 0),
                    b = t => {
