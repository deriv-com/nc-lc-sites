    ...
    i(t, "Series/Funnel/FunnelSeriesDefaults.js", [], function() {
        ...
        class S extends l {
            ...
            translate() {
                ...
                for (let t of M) {
                    if (X = null, w = A ? t.y / A : 0, R = (B = y - b / 2 + D * b) + w * b, T = (O = p - (F = e.getWidthAt(B)) / 2) + F, E = (Y = p - (F = e.getWidthAt(R)) / 2) + F, h(B) >= C ? (O = Y = p - m / 2, T = E = p + m / 2) : R > C && (X = R, E = (Y = p - (F = e.getWidthAt(C)) / 2) + F, R = C), n && (B = 2 * y - B, R = 2 * y - R, null !== X && (X = 2 * y - X)), j && ("point" === v || 0 === t.index || t.index === M.length - 1 || null !== X)) {
                        let e = Math.abs(R - B),
                            i = T - E,
                            s = E - Y,
                            n = Math.sqrt(i * i + e * e);
                        H = Math.atan(0 !== i ? e / i : 1 / 0), I = n / 2, null !== X && (I = Math.min(I, Math.abs(X - R) / 2)), s >= 1 && (I = Math.min(I, s / 2));
                        let a = W(H);
                        if (k = "stack" === v && 0 !== t.index ? [
                            ...
                            let e = W(Math.PI / 2);
                            a = W(Math.PI / 2 + H), k.push(["L", E + a.dx[0], R - a.dy[0]], ["C", E + a.dx[1], R - a.dy[1], E, R + a.dy[2], E, R + a.dy[3]]), "stack" === v && t.index !== M.length - 1 ? k.push(["L", E, X], ["L", Y, X]) : k.push(["L", E, X - e.dy[3]], ["C", E, X - e.dy[2], E - e.dx[2], X, E - e.dx[3], X], ["L", Y + e.dx[3], X], ["C", Y + e.dx[2], X, Y, X - e.dy[2], Y, X - e.dy[3]]), k.push(["L", Y, R + a.dy[3]], ["C", Y, R + a.dy[2], Y - a.dx[1], R - a.dy[1], Y - a.dx[0], R - a.dy[0]])
                        } else s >= 1 ? (a = W(Math.PI - H), "stack" === v && 0 === t.index ? k.push(["L", E, R], ["L", Y, R]) : k.push(["L", E + a.dx[0], R - a.dy[0]], ["C", E + a.dx[1], R - a.dy[1], E - a.dx[2], R, E - a.dx[3], R], ["L", Y + a.dx[3], R], ["C", Y + a.dx[2], R, Y - a.dx[1], R - a.dy[1], Y - a.dx[0], R - a.dy[0]])) : (a = W(Math.PI - 2 * H), k.push(["L", Y + a.dx[0], R - a.dy[0]], ["C", Y + a.dx[1], R - a.dy[1], Y - a.dx[1], R - a.dy[1], Y - a.dx[0], R - a.dy[0]]))
                    } else k = [
                        ["M", O, B],
                        ["L", T, B],
                        ["L", E, R]
                    ...
                    k.push(["Z"]), t.shapeType = "path", t.shapeArgs = {
                        d: k
                    }, t.percentage = 100 * w, t.plotX = p, t.plotY = (B + (X || R)) / 2, t.tooltipPos = [p, t.plotY], t.dlBox = {
                        x: Y,
                        y: B,
                        topWidth: T - O,
                        bottomWidth: E - Y,
                        ...
                        width: NaN
                    }, t.slice = d, t.half = P, t.isValid() && (!a || !1 !== t.visible) && (D += w)
                }
                c(e, "afterTranslate")
            }
            sortByAngle(e) {
