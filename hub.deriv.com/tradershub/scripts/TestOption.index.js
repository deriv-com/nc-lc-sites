require(["tslib"], function(tslib) {
    require(["@outsystems/runtime-core-js", "TestOption.appDefinition", "@outsystems/runtime-core-js/debugger"], function(OSRuntimeCore, TestOptionAppDefinition, NullDebugger) {
        var OS = OSRuntimeCore;
        if (OS.Navigation.ensureRequestSecurity()) {
            return;
        }

        Promise.all([OS.Application.default.initialize(TestOptionAppDefinition, OS.Interfaces.Application.InitializationType.Full, new OS.Format.DateTimeFormatInfo("yyyy-MM-dd", "HH:mm:ss"), new OS.Format.NumberFormatInfo(".", "")).then(function(success) {
            if (success) {
